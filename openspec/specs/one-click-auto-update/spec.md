...
为用户提供「下载 → 替换 → 重启」闭环的一键更新能力：触发「立即更新」后自动下载新版本 exe 资产（带进度反馈），下载完成后弹原生确认对话框，确认后自动退出、由一次性批处理脚本替换可执行文件并重启为新版本。包含目录可写探测与降级（跳转 GitHub 手动更新）、待更新状态复用（「重启更新」）、替换失败兜底（回退旧版本）与下载残渣清理。
